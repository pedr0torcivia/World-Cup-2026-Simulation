...
def write_outputs(
    ...
    (output_dir / "full_report.md").write_text(build_markdown_report(probability_tables, injuries_summary, metadata), encoding="utf-8")


def build_markdown_report(probability_tables: dict[str, pd.DataFrame], injuries_summary: pd.DataFrame, metadata: dict) -> str:
